...
class Core_Model_Upload_Document
{
    ...
    public function uploadArray($file)
    {
        ini_set('memory_limit', '-1');
        ini_set('max_execution_time', 300);
    
        // Comprobar si hay errores en el archivo
        if ($file["error"] > 0) {
            return false;
        } else {
            $permitidos = [
            ...
            if (in_array($file['type'], $permitidos) && $file['size'] <= $limite_kb * 1024) {
                $filename = $this->clearName(pathinfo($file['name'], PATHINFO_FILENAME));
    ...
    private function clearName($str)
    {
        ...
        $str = str_replace(array_map("utf8_encode",$tildes),$vocales,$str);
        $str = str_replace(array_map("utf8_decode",$tildes),$vocales,$str);
        //Quitar símbolos
        $simbolos = array("=","¿","?","¡","!","'","%","$","€","(",")","[","]","{","}","*","+","·",".","&lt; ","&gt;");
        $i = 0;
        while($simbolos[$i]){
