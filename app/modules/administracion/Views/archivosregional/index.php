        ...
        <div class="content-dashboard">
            <div class="row">
				<div class="col-3">
	                ...
	                <input type="text" class="form-control" name="archivo_fecha" value="<?php echo $this->getObjectVariable($this->filters, 'archivo_fecha') ?>"></input>
	                    </label>
	            </div>
				<div class="col-3">
		            <label>Archivo</label>
		            ...
		            <input type="text" class="form-control" name="archivo_archivo" value="<?php echo $this->getObjectVariable($this->filters, 'archivo_archivo') ?>"></input>
		            </label>
		        </div>
				<div class="col-3">
					<label>Usuario</label>
	                <label class="input-group">
	                    ...
	                    <select class="form-select" name="archivo_usuario">
	                        <option value="">Todas</option>
	                        <?php foreach ($this->list_archivo_usuario as $key => $value) : ?>
	                            <option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'archivo_usuario') ==  $key) { echo "selected";} ?>><?= $value; ?></option>
	                        <?php endforeach ?>
	                    </select>
	               </label>
	            </div>
		...
		<table class=" table table-striped  table-hover table-administrator text-start">
			...
			<tbody>
				...
				<?php $id =  $content->archivo_id; ?>
					<tr>
						<td><?=$content->archivo_fecha;?></td>
						<td><?=$content->archivo_archivo;?></td>
						<td><?= $this->list_archivo_usuario[$content->archivo_usuario];?>
						<td class="text-end">
							<div>
