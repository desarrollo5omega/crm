...
class Page_tableroController extends Page_mainController
{
    ...
    public function indexAction() {
		...
		if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			echo $this->_view->getRoutPHP('modules/page/Views/tablero/index.php'); // Carga solo la vista
			$this->_view->no = "d-none";
		}
        
    }

	public function getRequerimientosAction()
	{
		$proyecto = $this->_getSanitizedParam('proyecto');
		$requerimientosModel = new Administracion_Model_DbTable_Requerimientos();
		$requerimientos = $requerimientosModel->getList(" requerimientos_proyecto = '".$proyecto."' ","");
	...
	public function getSeccionAction()
	{
	...
	public function insertSoporteAction() {
		$this->setLayout('blanco');
		$response = ["success" => false, "message" => "Error desconocido"]; // Respuesta inicial
		...
		try {
			...
			// Insertar en tabla de ingenieros
			$datos = [
				"proyectosing_user" => $proyectosing_user,
				"proyectosing_observacion" => "",
				"proyectosing_proyecto" => $id
			];
			...
			$logData = [
				"id" => $id,
				"log_log" => print_r($data, true),
				"log_tipo" => "CREAR PROYECTO"
			];
	...
	private function getDataSoporte()
	{
		$data = array();
	...
	public function updateSeccionAction()
	{
		$this->setLayout('blanco');
