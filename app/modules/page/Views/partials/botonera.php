...
<?php
	...
	if ($_SESSION["kt_login_level"] == 5) {
		$sessionIng = $_SESSION["kt_login_id"];
	}
?> 
...
<div class="header-content w-100">
	...
	<div class="solo_cel">
		<div class="row m-0">
			<div class="col-lg-12">
				<div class="row">
					<div class="col-2 px-3 d-flex justify-content-start align-items-center">
						<div class="boton_menu" onclick="$('#div_menu').toggle();">
							<i class="fas fa-bars"></i>
						</div>
					</div>
					<div class="col-8">
						<a href="#" class="d-flex justify-content-center align-items-center">
							<img src="/corte/logo_crm.jpg" class="logo_crm img-fluid">
						</a>
					</div>
					<div class="col-2 px-3 d-flex justify-content-start align-items-center">
						<div class="boton_menu_graf">
		...
		<div id="div_menu" class="full-width" style="display: none;">
			<ul class="list-unstyled m-0">
				<li class="<?= $mod_dash ?>">
					<a href="/page/sistema/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/sistema") !== false){ echo 'activo'; } ?>">
							<i class="fas fa-home me-2"></i> Inicio
						</button>
					</a>
				</li>
				<li class="<?= $mod_clientes ?>">
					<a href="/page/clientes/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/clientes") !== false){ echo 'activo'; } ?>">
							<i class="fas fa-users me-2"></i> Clientes
						</button>
					</a>
				</li>
				<li class="<?= $mod_cotizaciones ?>">
					<a href="/page/proyectos/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/proyectos/") !== false){ echo 'activo'; } ?>">
							<i class="fas fa-list-alt me-2"></i> Cotizaciones
						</button>
					</a>
				</li>
				<li class="<?= $mod_proyectos ?>">
					<a href="/page/proyectosd/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/proyectosd") !== false){ echo 'activo'; } ?>">
							<i class="fas fa-list-alt me-2"></i> Proyectos
						</button>
					</a>
				</li>
				<li class="<?= $mod_reportes ?>">
					<a href="/page/reportes/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/reportes") !== false){ echo 'activo'; } ?>">
							<i class="fas fa-database me-2"></i> Reportes
						</button>
					</a>
				</li>
				<li class="<?= $mod_soportes ?>">
					<a href="/page/soporte/" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/soporte") !== false){ echo 'activo'; } ?>">
							<i class="fa-solid fa-ticket me-2"></i> Soportes
						</button>
					</a>
				</li>
				<li class="<?= $mod_tablero ?>">
					<a href="/page/tablero/?ing=<?= $sessionIng ?>" class="text-decoration-none">
						<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/tablero") !== false){ echo 'activo'; } ?>">
							<i class="fa-solid fa-layer-group me-2"></i> Plan de trabajo
						</button>
					</a>
				</li>
				<?php if($_SESSION['kt_login_level']=="1"){ ?>
					<li class="<?= $mod_usuarios ?>">
						<a href="/page/usuario/" class="text-decoration-none">
							<button class="btn-menu w-100 <?php if(strpos($_SERVER['REQUEST_URI'], "/usuario") !== false){ echo 'activo'; } ?>">
								<i class="fas fa-male me-2"></i> Usuarios
							</button>
						</a>
					</li>
				<?php } ?>
	...
	<div class="row no_cel">
		<div class="col-12 fondo_azul_oscuro py-1">
			<div class="d-flex justify-content-between">
				<div class="d-flex align-items-center">
					<!-- Sección del logo -->
					<a href="#" class="no_cel me-3 pe-3 alogo">
						<img src="/skins/page/images/solo_o.png" class="logo_crm" alt="Logo">
					</a>
					
						<?php
							$menu_items = [
								"Dashboard" => ["/page/sistema", $mod_dash, "fa-solid fa-table-columns"],
								"Clientes" => ["/page/clientes", $mod_clientes,"fa-solid fa-building icon-dash"],
								"Cotizaciones" => ["/page/proyectos", $mod_cotizaciones,"fa-solid fa-barcode icon-dash"],
								"Proyectos" => ["/page/proyectosd", $mod_proyectos,"fa-solid fa-diagram-project icon-dash"],
								"Reportes" => ["/page/reportes", $mod_reportes,"fa-solid fa-chart-line"],
								"Soportes" => ["/page/soporte", $mod_soportes,"fa-solid fa-headset"],
								"Plan de trabajo" => ["/page/tablero?ing=$sessionIng", $mod_tablero,"fa-solid fa-calendar-week"],
								"Usuarios" => ["/page/usuario", $mod_usuarios,"fa-solid fa-users-rectangle"]
							];

							// Obtener solo la ruta de la URL actual sin parámetros
							$current_path = strtok($_SERVER['REQUEST_URI'], '?'); // Esto corta la URL antes del '?'
							
							foreach ($menu_items as $name => [$url, $mod, $icon]) {
								$active_class = ($current_path == $url) ? 'activo' : '';
								echo '<a class="text-decoration-none ' . $mod . ' ' . $active_class . '" href="' . $url . '" data-bs-toggle="tooltip"  data-bs-placement="bottom"title="' . $name . '">
										<button type="button" class="boton_menu2 enlace_blanco"><i class="' . $icon . '"></i></button>
									</a>
									<span class="ms-3 grap-line ' . $mod . '"></span>';
							}
						?>
					
				</div>
				<div class="text-end d-flex justify-content-end h-100 px-4" style="height: 100vh;">

						<div class="d-flex justify-content-center align-items-center">
							<div>
								<b class="enlace_blanco titulo_bienvenido">Bienvenido(a): </b>
								<span class="enlace_blanco"><?php echo $_SESSION['kt_login_name']; ?></span>
							</div>
							<div class="d-inline-block no_cel align-content-center me-2 ms-2 div-exit" style="vertical-align:bottom;"></div>
							<div class="d-inline-block text-start ">
								<a href="/page/login/logout/" style="margin-left: 10px;" class="ano_cel">
									<i class="fa-solid fa-arrow-right-from-bracket icon-exit"></i>
									<br class="no_cel">
									<span style="margin-left:10px;" class="no_cel enlace_blanco">Salir</span>
								</a>
							</div>
...
function formatoDMYH($f) {
    // Verificar si $f es nulo o está vacío
    if (empty($f)) {
        return ''; // Retorna un valor vacío o un mensaje específico si no existe la fecha
    }

    // Separar la fecha y la hora
    $fechaHora = explode(" ", $f);
    $fecha = $fechaHora[0]; // Parte de la fecha
    $hora = isset($fechaHora[1]) ? $fechaHora[1] : ''; // Parte de la hora, si existe

    // Formatear la fecha a DD/MM/YYYY
    $aux = explode("-", $fecha);
    $fechaFormateada = $aux[2] . "-" . $aux[1] . "-" . $aux[0];
	...
	//$cuotaextra = str_replace('.','',$extra);
			var numerocuotasextra =  abonos;
				...
				if(hoy <= anio+'-06-30'){
					...
					//console.log("entro2")
					meses = '<?php echo (0)?>';
					console.log("mesd: "+ <?php echo (date('m'))?>);
				...
				console.log("error: "+numerocuotasextra2[12]);
	var seleccionado='';
...
function validar_fecha_expedicion(){
	...
	$("#error1").html();
	if(fecha2 <= fecha){
		var mensaje = "La fecha de expedici��n del documento debe ser mayor a la fecha de nacimiento";
		$("#error1").html('<div class="alert alert-danger error"><i class="far fa-times-circle error"></i> '+mensaje+'</div>');
